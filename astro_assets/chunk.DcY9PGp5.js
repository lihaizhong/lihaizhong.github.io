...
import { $ as $$BaseHead, a as $$Header, b as $$Footer } from './chunk.CyTgccFm.js';
...
const $$PostLayout = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`<html lang="en"> <head>${renderComponent($$result, "BaseHead", $$BaseHead, { "title": title, "description": description })}${renderHead()}</head> <body> ${renderComponent($$result, "Header", $$Header, {})} <main> <article> ${heroImage ? renderTemplate`<div> <img${addAttribute(heroImage, "src")} alt=""> </div>` : null} <h1 class="text-3xl my-1">${title}</h1> ${renderComponent($$result, "FormattedDate", $$FormattedDate, { "date": pubDate })} ${updatedDate ? renderTemplate`<div class="italic">
Last updated on ${renderComponent($$result, "FormattedDate", $$FormattedDate, { "date": updatedDate })} </div>` : null} <hr> ${renderSlot($$result, $$slots["default"])} </article> </main> ${renderComponent($$result, "Footer", $$Footer, {})} </body></html>`;
